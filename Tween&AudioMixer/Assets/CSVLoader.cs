...
public class CSVLoader : MonoBehaviour
{
    ...
    public void LoadCSV<T>(out T[] OutputArr, string path) where T : new()
    {
        ...
        for (int i = 0; i < lineData.Length - 2; i++)
        {
            ...
            OutputArr[i] = new T();
            Type type = typeof(T);
            System.Reflection.FieldInfo[] fields = type.GetFields();//필드정보가져옴
            int fieldID = 0;
            ...
            for(int j = 0;  j < fields.Length; j++)
            {
                ...
                {
                    if (currentLineSplited[fieldID].Equals("NONE"))//csv에 NONE이라고 넣어야됨 
                    {
    ...
    private string[] GenerateLineSplit(string currentLine)
    {
        ...
        if (currentLine.IndexOfAny(MarkArr) >= 0)
        {
            ...
            bool isQuot, isCurly, isSquare;//따움표, 중괄호, 대괄호 
