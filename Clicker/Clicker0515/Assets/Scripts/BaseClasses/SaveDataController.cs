...
public class SaveDataController : MonoBehaviour
{
    ...
    protected void FixSaveData()//세이브데이타 수정할때 
    {
        if (mUser.PlayerItemLevelArr.Length < Constants.PLAYER_ITEM_COUNT)
        {
            int[] temp = new int[Constants.PLAYER_ITEM_COUNT];
            //int count = Mathf.Min(Constants.PLAYER_ITEM_COUNT, mUser.PlayerItemLevelArr.Length); 적은값만 
            for (int i = 0; i < mUser.PlayerItemLevelArr.Length; i++)//i<count로 수정하여 적은값만큼만 반복
            {
