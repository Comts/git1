...
public class UIElement : MonoBehaviour
{
    ...
    public void Init(int id, Sprite icon, string title, string level, string contents, string cost, string tenCost, Delegates.TwoIntInVoidCallback callback)
    {
        mID = id;
    ...
    private IEnumerator ButtonPop()
    {
        ...
        while(time>0)
        {
            ...
            time -= .1f;
            if(mClickCount>=3) //3번 카운트되면 10업 버튼 등장
            {
                activeButton = true&&mTenUpButton.interactable; //&&mTenUpButton.interactable가 false이면 동작을 안 하게
